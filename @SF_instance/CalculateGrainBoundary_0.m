function obj = CalculateGrainBoundary(obj,app)
...
if ~obj.GrainBoundaryCalculateFlag
    ...
    for i = 1:ConnectedCellsNumber
        i1 = SeedsOriginalIndex(ConnectedCells1(i));
        i2 = SeedsOriginalIndex(ConnectedCells2(i));
        G1 =  obj.InvertTransferMatrix(3*(i1-1)+1:3*i1,:); % sam -> cry
        G2 =  obj.InvertTransferMatrix(3*(i2-1)+1:3*i2,:);
        dG = G2*G1'*D;
...
obj.GrainBoundaryPlot = [obj.GrainBoundaryPlot, ...
    showLine(GrainBoundaryEdgesVertex1,GrainBoundaryEdgesVertex2, ...
    obj.GrainBoundaryData.Vertexes,app.UIAxes,'black')];
...
function h = showLine(ind1,ind2,v,axe,color)
